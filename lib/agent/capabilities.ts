...
export const AGENT_CAPABILITY_CONTRACT = [
  {
    type: "task.create",
    purpose: "Create a new commitment or idea after understanding the user semantically.",
    policy: "proposal",
    notes:
  ...
  },
  {
    ...
    policy: "auto_or_proposal",
    notes: "Removal requires proposal; checking/unchecking and adding are reversible.",
  },
  {
    type: "fact.add/fact.update/fact.remove",
    purpose: "Preserve user knowledge, temporary context, corrections and explicit facts.",
    policy: "auto_or_proposal",
    notes:
  ...
  },
  {
    ...
    purpose: "Maintain real household members when the user supplies or corrects them.",
    policy: "auto_or_proposal",
    notes: "Never invent a person. Removal is destructive and requires proposal.",
  },
  {
    type: "homeArea.upsert/homeArea.remove",
    purpose: "Maintain real home areas explicitly stated or grounded in approved scan data.",
    policy: "auto_or_proposal",
    notes: "Removal is destructive and requires proposal.",
  },
  {
    type: "template.exclude/template.restore",
    purpose: "Respect explicit feedback that a home suggestion is irrelevant or relevant again.",
    policy: "auto",
    notes: "This changes suggestion eligibility, not natural-language interpretation.",
  },
  {
    type: "inventory.event",
    purpose: "Record a replenishment, depletion or correction event for forecast learning.",
    policy: "auto",
    notes:
...
export function buildAgentCapabilityContext() {
  return {
    ...
    categoryField: {
      ...
      validIds: CATEGORY_IDS,
      rule:
        "Choose from these only when semantic classification is clear; otherwise omit/unclassified. Do not use the list to decide what the user is allowed to mean.",
    },
  };
}
