-- When reminder.dueAt changes via AppState save, reschedule queue row and clear stale lease.
-- EXPAND-safe: replaces save_app_state body only; no table contract change.
-- Applied name suggestion: reminder_queue_reschedule_clear_lease
...
begin
  ...
  insert into reminder_queue(id, owner_id, due_at, status)
    ...
    on conflict (id) do update set
      ...
      lease_until = case
        when reminder_queue.due_at is distinct from excluded.due_at then null
        else reminder_queue.lease_until
      end
